...
From the cross-validation we quantified *Root Mean Squared Error*.
...
By itself, *DMK class* produced a concordance correlation coefficient of 0.17 (compared to 0.008 at Skrimfjella), but the worst RMSE of any model at either site: \unit{98\,cm}.

When we tested how well models extrapolated from within to outside of mapped peatland, we found that the models produced marginally better RMSE than just assuming a constant \unit{30\,cm} depth (Fig. \@ref(fig:modelMetricsExtrapolation)).
Compared to our error estimates with the full dataset, RMSE at Skrimfjella was nearly unchanged, while at Ørskogfjellet it was lower outside of mapped peatland.
