...
pNorway <- ggplot() +
  ...
  geom_sf(data = both, mapping = aes(color = 'red', fill = 'red')) +
  geom_text_repel(
    data = labels,
...
ar50aalesund <- st_read(
  "ms/figures/data/1508_25832_ar50_gdb/1507_25832_ar50_gdb.gdb"
)
ar50vestnes <- st_read(
  "ms/figures/data/1535_25832_ar50_gdb/1535_25832_ar50_gdb.gdb"
)
...
mapOrskog <- ggplot(ar50orskog) +
  ...
  scale_fill_discrete(
    ...
    type = CBpaired.6class
  ) +
  geom_sf(
    data = contourlinesOrskog,
    linewidth = 0.2,
    color = "#fdbf6f",
    show.legend = FALSE
  ) +
...
ar50kongsberg <- st_read(
  "ms/figures/data/3303_25833_ar50_gml/3006_25833_ar50_gml.gml",
  "ArealressursFlate"
) |>
  st_set_crs(25833)
...
# Creating hillshade
dtm <- c(
  "ms/figures/data/940334_dtm50/data/dtm50_6601_50m_33.tif",
...
mapOrskog <- ggplot(orskog) +
  ...
  scale_fill_distiller(palette = "Greys") +
  geom_sf(data = modelOrskog, mapping = aes(color = depth_cm), alpha = 1) +
...
label_data <- data.frame(
  ...
  y = y_values,
  label = c(
    paste("mean:", round(mean_depth), "cm"),
...
densityOrskog <- ggplot(modelOrskog, aes(x = depth_cm)) +
  ...
  scale_x_continuous(
    breaks = seq(0, max(modelOrskog$depth_cm), by = 200),
...
mapSkrim <- ggplot(skrim) +
  ...
  scale_fill_distiller(palette = "Greys") +
  geom_sf(data = modelSkrim, mapping = aes(color = depth_cm), alpha = 1) +
  scale_color_viridis_c(limits = color_limits, direction = -1, guide = "none") +
  geom_sf(
    data = skrim,
    color = 'black',
...
densitySkrim <- ggplot(modelSkrim, aes(x = depth_cm)) +
  ...
  scale_fill_viridis_c(limits = color_limits, direction = -1, guide = "none") +
  scale_x_continuous(
    breaks = seq(0, max(modelSkrim$depth_cm), by = 200),
    labels = scales::label_number(suffix = " cm")
  ) +
  scale_y_continuous(expand = c(0, 0)) +
  geom_text_repel(
...
## Ørskogfjellet with photo panel ####

photopath <- "ms/figures/foto-KKM.JPG"
...
photo <- magick::image_read(photopath) |>
  magick::image_annotate(
    "Photo: Karl-Kristian Muggerud",
    size = 80,
    color = "black",
    gravity = "northwest",
  ) |>
...
# Calculate height ratio based on aspect ratios to align widths
# Get photo aspect ratio
photo_info <- magick::image_info(magick::image_read(photopath))
photo_aspect <- photo_info$width / photo_info$height

# Get pOrskog aspect ratio from bounding box
bbox_width <- bboxOrskog[3] - bboxOrskog[1]
bbox_height <- bboxOrskog[4] - bboxOrskog[2]
pOrskog_aspect <- bbox_width / bbox_height

# Height ratio = pOrskog_aspect / photo_aspect (so widths align with same available width)
height_ratio <- pOrskog_aspect / photo_aspect

# Combine photo (top) with pOrskog map (bottom), aligning with plotting area
photomap <- photo / pOrskog + plot_layout(heights = c(height_ratio, 1))
ggsave(
  photomap,
  filename = 'map-orskog-photo.jpg',
  path = "ms/figures",
  width = 210 - 30,
  height = (297 - 30 - 40),
  units = 'mm',
...
g1 <- ggplot(plotting) +
  ...
  geom_point(
    aes(y = model, x = mean, color = site),
    position = position_dodge2(width = 0.3, reverse = TRUE)
  ) +
  geom_text(
    aes(y = model, x = mean, label = signif(mean, 2), group = site),
  ...
  scale_color_discrete(
    labels = c(orskog = "\u00D8rskogfjellet", skrim = "Skrimfjella")
  ) +
  theme_bw() +
  theme(
...
ggsave(
  g1,
  filename = 'modelmetrics-faceting.pdf',
  path = "ms/figures",
  width = 210 - 30,
  height = (240 - 40) / 2.5,
  units = 'mm'
) #copernicus.cls page 210x240
...
orskogQuantiles <- sf::st_read(
  "output/modeling.gpkg",
  layer = "quantiles.cv"
) |>
  sf::st_drop_geometry() |>
...
skrimQuantiles <- sf::st_read(
  "output/Skrim/modeling.gpkg",
  layer = "quantiles.cv"
) |>
  sf::st_drop_geometry() |>
  select(coverage, .pred_lower, .pred_upper, depth_cm) |>
...
scatter1 <- ggplot(orskog, aes(x = depth_cm, y = .pred)) +
  geom_segment(
    aes(x = depth_cm, xend = depth_cm, y = .pred_lower, yend = .pred_upper),
    color = "grey70",
    alpha = 0.2,
    linewidth = 0.2
  ) +
...
density2 +
  ...
  plot_layout(
    nrow = 2,
    byrow = TRUE,
    heights = unit(c(-1, 4), c("null", "null"))
  ) +
  plot_annotation(tag_levels = list(c('(a)', '(b)')))
ggsave(
  filename = 'calibration_plots.pdf',
  path = "ms/figures",
  width = (210 - 30),
  height = (240 - 40) / 2,
  units = 'mm'
...
create_simple_corr_plot <- function(corr_data) {
  corr_data %>%
    ...
    coord_flip() +
    theme_void() +
    theme(axis.text = element_text(size = 6), plot.margin = margin(2, 2, 2, 2))
}
...
pOrskog <- plot.orskog %>%
  ...
  geom_col(
    aes(
      x = fct_reorder(Variable, Imp.median),
      y = Importance,
      group = type,
      fill = type
    ),
  ...
  coord_flip() +
  scale_fill_discrete(
    labels = c(firm = "FIRM", perm.vip = "permutation", shap = "Shapley")
  ) +
  theme_minimal() +
  theme(
    axis.title.y = element_blank(),
...
ggsave(
  pAll,
  filename = 'variable_importance.pdf',
  path = "ms/figures",
  width = (210 - 30),
  height = (240 - 40) / 1.25,
  units = 'mm'
...
ggsave(
  filename = 'partial_dependence.pdf',
  path = "ms/figures",
  width = (210 - 30),
  height = (240 - 60),
  units = 'mm'
